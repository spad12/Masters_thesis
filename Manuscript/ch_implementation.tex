...
\end{algorithm}
	...
	When the particle advancing is accelerated by a factor of 70, the poisson solve becomes the major bottleneck. For sceptic3D leaving the poisson solve on the CPU resulted in only a factor of 10 speedup for the code. In order to achieve reasonable performance boost for the entire code we need to speed up the poisson solve by a factor of 20 or so. 



The field solve used in the serial version of sceptic3D involves solving the problem $Ax=b$ where $b$, the charge density, is an array of $n_r\times n_{\phi} \times n_{psi}$, $A$ is the stiffness matrix of the Poisson solver and $x$ is the potential. The biconjugate gradient method is used to solve this system, and since this method is merely a series of matrix multiplications it is easily parallelized on the GPU.  
...
\end{itemize}
	...
	For the single value quantities, such as the quantities of total momentum, energy, and particle count are tallied using atomic operations in shared memory. 
